...
void Leq_EliminateVarCadet(Abc_Ntk_t * &pNtk, unsigned uBegin, Leq_Ba_t * pBa){ //type 0 -> exist, 1-> universal
  ...
  if(Vec_IntSize(pScopeV)){
    pNtkV = cadetExistsElim(pNtkU, pScopeV);
    Abc_NtkDelete(pNtkU);
  }
  ...
  if(Vec_IntSize(pScopeN)){
    pNtkN = cadetExistsElim(pNtkV, pScopeN);
    Abc_NtkDelete(pNtkV);
  }
  pNtk = pNtkN;
